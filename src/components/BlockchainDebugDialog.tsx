    ...
    const jsonData = {
      ...
      gameRound: debug.gameRound
        ? {
            ...
            betAmounts: debug.gameRound.betAmounts
              ?.slice(0, debug.gameRound.betCount || 0)
      ...
      <div className="bg-gray-900 rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-purple-500/30">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700 bg-gray-800">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <RefreshCw
                className={`w-5 h-5 text-gray-300 ${debug.isLoading ? "animate-spin" : ""}`}
              />
            </button>
            <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <Section title="Connection">
            <InfoRow
              label="Status"
              value={debug.connected ? "Connected" : "Disconnected"}
              icon={
                debug.connected ? (
          ...
          {debug.gameConfig && (
            <Section title="Game Config">
              <InfoRow
                label="Authority"
                value={debug.gameConfig.authority?.toString() || "N/A"}
                mono
                copyable
              />
              <InfoRow
                label="Treasury"
                value={debug.gameConfig.treasury?.toString() || "N/A"}
                mono
                copyable
              />
              ...
              <InfoRow
                label="Bets Locked"
                value={debug.gameConfig.betsLocked ? "Yes" : "No"}
                icon={
                  debug.gameConfig.betsLocked ? (
                    <XCircle className="w-4 h-4 text-red-500" />
                  ) : (
                    <CheckCircle className="w-4 h-4 text-green-500" />
                  )
              ...
              <InfoRow
                ...
                value={`${debug.gameConfig.smallGameDurationConfig?.waitingPhaseDuration || "N/A"}s`}
              />
            </Section>
          )}
          ...
          {debug.gameRound && (
            <Section title="Active Game Round">
                  ...
                  <div className="space-y-2">
                    {Array.from({ length: debug.gameRound.betCount }).map((_, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="flex items-center justify-between bg-gray-700/30 rounded-lg px-3 py-2 border border-gray-600/50"
                        >
              ...
              <InfoRow
                label="Winner"
                value={debug.gameRound.winner?.toString() || "Not determined"}
                mono
              />
              <InfoRow
                label="VRF Request"
                value={debug.gameRound.vrfRequestPubkey?.toString() || "N/A"}
                mono
                copyable
              />
              <InfoRow
                label="Randomness Fulfilled"
                value={debug.gameRound.randomnessFulfilled ? "Yes" : "No"}
              />
            </Section>
          )}
